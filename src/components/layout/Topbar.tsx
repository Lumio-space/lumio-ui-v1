 ...
 *
 * Migrated from components/layout/Topbar.tsx.
 *
 * Key changes:
 *   - useNavigate → useRouter from next/navigation
 *   - Role / setRole from Zustand auth store (was React Context)
 *   - Avatar import from shared components (not ui/)
 *   - Custom Dropdown ported inline (shadcn DropdownMenu added in Phase 2)
 *   - 'use client' required: hooks + interactivity
 */
...
/* ─── Inline Dropdown (replaced by shadcn in Phase 2) ─── */
...
function Dropdown({ trigger, children, menuClassName }: DropdownProps) {
  ...
  return (
    <div ref={ref} className="relative">
      <button type="button" onClick={() => setOpen((o) => !o)} aria-haspopup="menu" aria-expanded={open} className="block">
        {trigger}
      </button>
      {open && (
        <div
          ...
          className={cn(
            'absolute right-0 z-40 mt-2 min-w-[12rem] overflow-hidden rounded-xl border border-slate-100 bg-white p-1.5 shadow-elevated',
            menuClassName
          )}
        >
          {children}
...
}: { icon?: React.ReactNode; danger?: boolean; className?: string; children: React.ReactNode } & React.ButtonHTMLAttributes<HTMLButtonElement>) {
  return (
    <button
      type="button"
      role="menuitem"
...
/* ─── Mock user data per role (replaced by session in Phase 2) ─── */

const ROLE_USERS: Record<LegacyRole, { name: string; email: string }> = {
  admin:   { name: 'Sarah Mitchell',   email: 'sarah.m@lumio.edu' },
  teacher: { name: 'Dr. Evelyn Hughes', email: 'evelyn.h@lumio.edu' },
  parent:  { name: 'Robert Bennett',   email: 'robert.b@gmail.com' },
};

const NOTIFICATIONS = [
  { id: 1, title: 'New admission request', desc: 'Grade 9 — pending review',     tone: 'gold',   time: '5m' },
  { id: 2, title: 'Attendance submitted',  desc: 'Grade 10-A by Dr. Hughes',     tone: 'purple', time: '1h' },
  { id: 3, title: 'Results published',     desc: 'Mid-term Grade 11-C',          tone: 'green',  time: '3h' },
] as const;
  ...
  return (
    <header className="sticky top-0 z-30 flex h-16 items-center gap-3 border-b border-slate-200 bg-white/90 px-4 backdrop-blur-md sm:px-6">
      ...
      <div className="flex flex-1 items-center justify-end gap-1.5 sm:flex-none">
        ...
        {/* Notifications */}
        <Dropdown menuClassName="w-80" trigger={
        ...
        <Dropdown trigger={
          ...
          <DropdownDivider />
          <DropdownItem danger icon={<LogOutIcon className="h-4 w-4" />} onClick={() => router.push('/login')}>
            Sign out
          </DropdownItem>
        </Dropdown>
      </div>
